...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS contacts (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, phone TEXT, email TEXT)''')
    conn.commit()
    return conn
...
def get_all_contacts_db(conn, search_term=" "):
    ...
    if search_term:
        cursor.execute(
            "SELECT id, name, phone, email FROM contacts WHERE LOWER(name) LIKE ?",
            (f"{search_term.lower()}%",)
)
